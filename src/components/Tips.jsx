    ...
    const tips =[
        {
            ...
            content: "Brush your teeth at a 45-degree angle to the gums using short, gentle strokes. Make sure to brush all surfaces of your teeth.",
            icon: <GiToothbrush className='w-8 h-8 text-sky-500'/>,
        
        },
        {
            title: "Limit Sugary Foods and Drinks",
            content: "Consuming too much sugar can lead to tooth decay. Try to limit your intake of sugary snacks and beverages.",
            icon: <FaShieldAlt className='w-8 h-8 text-amber-500'/>,
        },
        {
            title: "Regular Dental Check-ups",
            content: "Visit your dentist at least twice a year for professional cleanings and check-ups to maintain good oral health.",
            icon: <FaSmile className='w-8 h-8 text-pink-500'/>,
        },
        {
            title: "Stay Hydrated",
  ...
  return (
    <section id="tips" className='scroll-mt-20 max-w-6xl mx-auto px-4 py-12'>
          ...
          <p className='text-lg text-gray-600 max-w-2xl mx-auto'>
                Maintain a healthy smile with these essential dental care tips.
          </p>
           </div>
           ...
           mb-8'>
            {
                tips.map((tip,index)=>(
                 ...
                 className={`p-4 rounded-xl transition-all duration-300
                  flex flex-col items-center ${activeTip === index ? 
                  'bg-white shadow-lg border-b-4 border-sky-400' : 
                  'bg-gray-100 hover:bg-blue-200'}`}>
                    <div className='mb-2'>{tip.icon}</div>
                      <h3 className='font-medium text-gray-800 text-sm md:text-base'>{tips.title}</h3>
           ...
           <div className='mt-8 flex justify-center'>
               <div className='flex space-x-2'>
                     ...
                     ${activeTip === index ? 'bg-sky-500 w-6' : 'bg-gray-300'}`} aria-label={`Go to tip ${index + 1}`}>
